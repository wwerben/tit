...
// Pobieranie danych klientów na podstawie CustomerID
  ...
  <div class="flex w-full">
    ...
    <div class="p-4 mt-4 w-full">
      <div class="flex justify-between w-full">
        ...
        <form action="rezerwacje.php" method="get" class="mb-4 flex h-12 w-full">
                ...
                <?php if (!empty($reservations) && count($reservations) > 0): ?>
                    ...
                    <tr>
                        ...
                        <td class="py-2 px-4 border border-slate-600 text-center">
                          <button onclick="toggleDetails(<?= $reservation['ID'] ?>)" class="bg-blue-600 text-white py-2 px-3 rounded-md font-medium">Szczegóły</button>
                        </td>
                        <td class="py-2 px-4 border border-slate-600 text-center">
                          <a href="index.php?delete_id=<?= htmlspecialchars($reservation['ID']) ?>" class="bg-red-600 text-white font-medium rounded-md py-2 px-3" onclick="return confirm('Czy na pewno chcesz usunąć tę rezerwację?')">Usuń</a>
                        </td>
                    </tr>
                    <tr id="details-<?= $reservation['ID'] ?>" style="display: none;">
                        <td colspan="13" class="border border-slate-600">
                            ...
                            if ($customer):
                            ?>
                            <div class="p-4 grid grid-cols-4 text-center  max-w-4xl ">
                                
                                <div class=" border-black">
                                    <label class="text-lg font-semibold">Imie</label>
                                    <p> <?= htmlspecialchars($customer['firstName']) ?></p>
                                 </div>
                                 <div class=" border-black">
                                    <label class="text-lg font-semibold">Nazwisko</label>
                                    <p> <?= htmlspecialchars($customer['lastName']) ?></p>
                                 </div>
                                 <div class=" border-black">
                                 ...
                                 <div class=" border-black">
                                    <label class="text-lg font-semibold">Telefon</label>
